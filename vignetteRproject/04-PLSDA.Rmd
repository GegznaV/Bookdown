...
```{r 04-global_options, include=FALSE}
library(knitr)
# global options
...
```{r 04-overview-PLSDA, echo=FALSE, message=FALSE}
library(mixOmics)
coul <- color.mixo(1:3)
...
```{r 04-data-srbct,echo=TRUE, message=FALSE}
library(mixOmics)
data(srbct)
X <- srbct$gene
Y <- srbct$class 
...
```{r 04-splsda-and-plots, echo=FALSE,results='hide', fig.show = 'hide'}
...
```{r 04-plsda-and-plots, fig.show='hide'}
...
```{r 04-auroc, echo=FALSE,results='hide', fig.keep='all'}
...
```{r 04-splsda-selectVar-value, eval=TRUE}
...
```{r 04-splsda-3d, eval=TRUE, fig.show='hide'}
...
```{r 04-final-splsda, fig.show = 'hide'}
...
* Can I have a hierarchy between two factors (e.g. diet nested into genotype)?
    + Unfortunately no, sparse PLS-DA only allows discriminating all groups at once (i.e. 4 x 2 groups when there are 4 diets and 2 genotypes)
