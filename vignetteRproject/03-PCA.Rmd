...
```{r 03-overview-PCA, echo=FALSE, message=FALSE, fig.width=10}
library(mixOmics)
coul <- color.mixo(1:3)
...
```{r 03-pca-liver-2-plot, fig.width=7}
plot(MyResult.pca2)
MyResult.pca2
```
...
```{r 03-pca-liver-loadings}
# a minimal example
plotLoadings(MyResult.pca)
```

```{r 03-pca-liver-loadings-args}
...
Plots can also be interactively displayed in 3 dimensions using the option `style="3d"`. We use the `rgl` package for this (the interative figure is only interatice in the `html` vignette).

```{r 03-pca-liver-plotIndiv, webgl=TRUE}
...
```{r 03-pca-liver-comp2, echo=FALSE,results='hide', fig.show=TRUE}
...
```{r 03-tune-pca, eval=TRUE, echo=TRUE, results='hide'}
tune.pca(X)
```
