...
```{r 03-global_options, include=FALSE}
library(knitr)
# global options
knitr::opts_chunk$set(dpi = 100, echo=TRUE, warning=FALSE, message=FALSE, eval = TRUE, cache=TRUE,
                      fig.show=TRUE, fig.width= 7,fig.height= 6,fig.align='center', out.width = '50%', fig.pos= "h", out.extra = '',
                      fig.path= 'Figures/')
```
...
```{r 03-overview-PCA, echo=FALSE, message=FALSE}
library(mixOmics)
coul <- color.mixo(1:3)
...
*I would like to identify the major sources of variation in my data and identify whether such sources of variation correspond to biological conditions or experimental bias. I would like to visualise trends or patterns between samples, whether they 'naturally' cluster according to known biological conditions.* 
</span>
...
```{r 03-liver-data, echo=TRUE, message=FALSE}
library(mixOmics)
data(liver.toxicity)
...
```{r 03-pca-liver-1,eval=TRUE, fig.show=FALSE}
...
```{r 03-pca-liver-2, eval=TRUE, fig.show=TRUE}
...
### Amount of variance explained and choice of a number of components
...
```{r 03-pca-liver-loadings,eval=TRUE, fig.show = 'hide'}
# a minimal example
plotLoadings(MyResult.pca)
```

```{r 03-pca-liver-loadings-args, eval=TRUE, fig.show = 'hide'}
...
```{r 03-pca-liver-plotIndiv, eval= TRUE, fig.show = 'hide'}
...
```{r 03-pca-liver-comp2, echo=FALSE,results='hide', fig.show=FALSE}
...
```{r 03-tune-pca, eval=TRUE, echo=FALSE, results='hide'}
tune.pca(X)
```

Regarding the number of variables to select in sparse PCA, there is no clear criterion at this stage. As PCA is an exploration method, we prefer to set arbitrary thresholds that will pinpoint the key variables to focus on during the interpretation stage.
